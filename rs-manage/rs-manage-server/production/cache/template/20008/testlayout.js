oojs.define({
    ...
    render: function (requestInfo) {
        ...
        // 检索出的广告数
        var adCount = ads.mainCreatives.length;

        var wapAppInfo = requestInfo.wapAppInfo;
        // 多广告模板
        for (var i = 0; i < adCount; i++) {
            ...
            // extention
            if (ad.extention) {
                ext = JSON.parse(wapAppInfo);
            }
            // app探测相关
            var deteConf = ext.deteConf || {};
            var appData = ext.appData || {};
