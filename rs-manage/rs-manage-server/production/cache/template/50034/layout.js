...
oojs.define({
    ...
    drawTuWen: function (requestInfo, itemList, style, ad, adIndex) {
       ...
       /* if (requestInfo.adElements.length === 1) {
        ...
        if (isVertical) {
            containerHeight = Math.min(containerStyle.width, containerStyle.height);
       ...
       // logo单独一行
        ...
        style['.liCon' + adIndex + ' .desc'] = {
            'width': textWidth,
            'height': Math.round(26 * pt) * 2 + 2,
            'margin': itemPadding / 2 - 2 + 'px ' + itemPadding + 'px',
            'text-align': 'center'
        };
    ...
    drawImage: function (requestInfo, itemList, style, ad, adIndex) {
        ...
        if ((adsLength > 1) || imgRadio !== 2) {
            ...
            itemBg.class = 'itemBg';
            itemBg.id = 'itemBg';
            itemList.childNodes.push(itemBg);
    ...
    render: function (requestInfo) {
        ...
        var clientHeight = containerStyle.height;
        if (isVertical) {
            clientHeight = Math.min(containerStyle.width, containerStyle.height);
        ...
        for (var i = 0, len = ads.length; i < len; i++) {
            ...
            var itemListObj = {};
            var stuffType = 0;
            if (ad.imgFileSrc.length > 0) {
                stuffType = 100;
            }
            else if ((ad.textTitle > 0 || ad.textDesc1.length > 0)
            && ad.iconFileSrc.length > 0) {
                stuffType = 600;
            }
            else if (ad.textTitle > 0 || ad.textDesc1.length > 0) {
                stuffType = 500;
