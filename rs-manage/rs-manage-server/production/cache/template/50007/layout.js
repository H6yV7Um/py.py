...
oojs.define({
    ...
    render: function (requestInfo) {
        ...
        var positionLR = (ext.pdp && ext.pdb === 3) ? 'right' : 'left';
        var imgW = ext.pictureWidth || 0.2 * containerWidth;
        var imgH = ext.pictureHeight || containerHeight;
        var imgTitleDis = ext.marginTitlePicture * containerWidth || ext.marginTitlePicture
        * containerWidth || 5;
        var titleMTop = ext.paddingTitleTop * containerWidth || 5;
        ...
        var descMTop = 0.1 * containerHeight;
        ...
        if (sourceT.replace(/[^\x00-\xff]/g, 'ci').length / 2 > titleLen) {
            sourceT = sourceT.substr(0, titleLen).replace(/([^\x00-\xff])/g, '\x241 ').substr(0, titleLen * 2
            - 4).replace(/[^\x00-\xff]$/, '').replace(/([^\x00-\xff]) /g, '\x241') + '…';
        ...
        if (sourceD.replace(/[^\x00-\xff]/g, 'ci').length / 2 > descLen) {
            sourceD = sourceD.substr(0, descLen).replace(/([^\x00-\xff])/g, '\x241 ').substr(0, descLen * 2
            - 4).replace(/[^\x00-\xff]$/, '').replace(/([^\x00-\xff]) /g, '\x241') + '…';
            adDesc.innerHTML = sourceD;
        }
        var adLeftPadding = positionLR === 'right' ? ('0px 0px 0px' + imgTitleDis + 'px') : ('0px '
        + imgTitleDis + 'px 0px 0px');
