...
oojs.define({
    ...
    $page: function () {
        try {
            ...
            if (window.adsExtention && document.getElementById('container') && this.sdk) {
            ...
            var fbIconDiv = document.querySelector('#fbIconDiv'); // 关闭反馈小红点
            var fbTipDiv = document.querySelector('#fbTipDiv'); // 关闭反馈提示
            ...
            if (fbIconDiv) {
                fbIconDiv.style.left =  mobbdadIcon.style.left;
                fbIconDiv.style.bottom = mobbdadIcon.style.bottom;
            }

            if (fbTipDiv) {
                fbTipDiv.style.left =  mobbdadIcon.style.left;
                fbTipDiv.style.bottom = mobbdadIcon.style.bottom;
            }
    ...
    onAdClick: function (event) {
        ...
        if (act === 2) {
            ...
            // 否则广告的点击串是 mcurl
                ...
                // var ck = clickUrl.split('&')[1] || '';
                var container = document.getElementById('container');
                adInfo = this.addVideoInfo(adInfo);
                // adInfo.v_url = this.getAttr(container, 'data-mcurl') + '&' + ck;;
                adInfo.v_url = this.getAttr(container, 'href');
            }
        } else {
            adInfo = this.addVideoInfo(adInfo);
