...
oojs.define({
    ...
    render: function (requestInfo) {
        ...
        var closeScale = 20 / 3;
        var u = requestInfo.device.userAgent;
        if (aspectRatio && aspectRatio === closeScale
            && u.indexOf('Android') > -1 || u.indexOf('Adr') > -1) {
            style['.icon'] = {
                'margin': (iOS ? '1.9375' : '0.9375') + 'rem ' + (iOS ? '3.575' : '0.789') + 'rem 0 0',
                'width': '5.25rem',
                'height': '5.25rem',
                'border-radius': '3.125rem',
                'float': 'right',
            ...
            style['.text'] = {
                'float': 'left',
                'margin-left': withImage ? '0.9375rem' : '0.4687rem'
            };
