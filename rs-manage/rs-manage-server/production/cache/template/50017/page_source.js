...
oojs.define({
    ...
    getClass: function (element, styleName) {
        if (element.currentStyle) {
            return element.currentStyle[styleName];// IE下获取非行间样式
        }
        else {
            return getComputedStyle(element, false)[styleName];// FF、Chorme下获取费行间样式
        }
    },
    // 绑定广告点击
    ...
    onAdClick: function (event) {
        ...
        if (adAct === 2 && m.indexOf('safari') > -1 && m.indexOf('version') > -1
