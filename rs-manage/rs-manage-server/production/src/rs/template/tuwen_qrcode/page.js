...
oojs.define({
    ...
    $page: function () {
        try {
            ...
            document.getElementsByTagName('html')[0].style.fontSize = width / 16 + 'px';
            ...
            if (downBtn) {
                downIndex = this.getAttr(downBtn, 'downIndex');
                var downRemoveStr = this.getAttr(downBtn, 'remove');
                if (downRemoveStr) {
                    var downRemoveArr = downRemoveStr.split(',');
                    for (var i = 0; i < downRemoveArr.length; i++) {
                        winUrlArr.splice(downRemoveArr[i], 1);
                    }
                }
                this.initPage2();
            }
    ...
    onAdClick: function (event) {
        ...
        downLoadTimer = new Date().getTime();
    ...
    getAppid: function () {
        return adsExtention[downIndex].appinfo.promoteAppId;
    },
    getAnswer: function () {
    ...
    checkAnswerIsSuccess: function (answer) {
        ...
        var appid = adsExtention[downIndex].appid;
    ...
    computeReward: function (answerIsCorrect) {
        var ad = adsExtention[downIndex];
        var cpc = adsExtention[downIndex].bid;
    ...
    checkAnswerHandler: function (result) {
        ...
        this.sendEclickRequest('//qrcode-logs.baidu.com/ad.jpg', {
            url2: 'qrcode',
            tel: telNumber,
            charge: this.charge,
            ad_s: ad.ad_s, // qk
    ...
    createPassportScript: function (callback) {
        ...
        var s = document.createElement('script');
        s.type = 'text/javascript';
        s.src = '//wappass.baidu.com/static/touch/js/api/wrapper.js?cdnversion=' + (+new Date());
    ...
    passportOnloadCallback: function () {
        ...
        passport.use('smsLogin', {
            ...
            var loginInstance = new apiMagic.passport.smsLogin({
                product: 'display_ads',
                staticPage: '//pos.baidu.com/v3Jump.html', // 跨域用静态页面
                u: '//m.baidu.com' // 登录成功跳转的目标地址
            });
