...
oojs.define({
    ...
    render: function (context) {
        ...
        for (var i = 0, len = ads.length; i < len; i++) {
            ...
            // 按比例计算图片的尺寸
            if (ads[i].height > containerHeight / adLen) {
                var tuwenLogoheight = containerHeight / adLen;
                var tuwenLogoWidth = tuwenLogoheight * ads[i].width / ads[i].height;
                if (tuwenLogoWidth > containerWidth / 3) {
                    tuwenLogoWidth = containerWidth / 3;
                    tuwenLogoheight = tuwenLogoWidth * ads[i].height / ads[i].width;
                }
            } else {
                var tuwenLogoheight = ads[i].height;
                var tuwenLogoWidth = tuwenLogoheight * ads[i].width / ads[i].height;
                if (tuwenLogoWidth > containerWidth / 3) {
                    tuwenLogoWidth = containerWidth / 3;
                    tuwenLogoheight = tuwenLogoWidth * ads[i].height / ads[i].width;
            ...
            // 按照图片的原尺寸
            // var tuwenLogoheight = ads[i].height;
            // var tuwenLogoWidth = ads[i].width;
        ...
        items.push(qiushiInfo);
         // 添加logo
        if (fullConfig.isShowTuiGuang) {
            /*var tuiGuang = engine.getLayout(fullConfig);
            tuiGuang.tagName = 'div';
            tuiGuang.id = 'tuiGuang';
            tuiGuang.class = 'tuiGuang';

            tuiGuang.innerHTML = '推广';
            container.childNodes.push(tuiGuang);

            style['.tuiGuang'] = {
                'position': 'absolute',
                'bottom': '0px',
                'right': '27px',
                'width': fullConfig.tuiGuangFontSize * 2 + 6 + 'px',
                'height': fullConfig.tuiGuangFontSize + 4 + 'px',
                'font-size': fullConfig.tuiGuangFontSize + 'px',
                'background-color': '#' + fullConfig.tuiGuangBackgroundColor,
                'color': '#' + fullConfig.tuiGuangFontColor,
                'text-align': 'center'
            };*/
        }
        else if (fullConfig.logoIsShow) {
