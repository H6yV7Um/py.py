/**
 * @file template.tuwen_wap_feed_image_2 template page event
 * @author  nieyuxin
 */
 /*global oojs */
oojs.define({
    ...
    onAdClick: function (event) {
        ...
        // 分10%的流量发送elick日志,日志包含styleid和act字段
        var act = this.getAttr(sourceElement, 'data-adtype');
        var temp = new Date().getTime();
        if (Math.random() * 100 < 10) {
            var eclickLog = 'http://eclick.baidu.com/puzzle.jpg?' + 'styleid='
            + 4030 + '&act=' + act + '&T=' + temp;
            this.sendByImage(eclickLog);
        }
