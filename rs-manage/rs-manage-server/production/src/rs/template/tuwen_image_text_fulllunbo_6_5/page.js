/**
 * @file tuwen_image_text_fulllunbo_6_5 template page event
  *@author qianxiaoli
 */
    ...
    doCircle: function (num, isNext) {
        ...
        if (isNext) {
            if (num < ulLen - 1) {
                ...
                itemUl.style.transition = 'left .5s ease-in-out';
                num++;
            }
            else {
        ...
        var itemWidth = parseInt(document.getElementById('container').clientWidth, 10);
        var itemHeight = parseInt(document.getElementById('container').clientHeight, 10);
        if (itemWidth * 5 / 6 > itemHeight) {
            itemWidth = itemHeight * 6 / 5;
        }
        var rem = itemWidth / 16;
        // itemCircleChildren[num].style.backgroundPosition = '0px  7.75px';
...
oojs.define({
        ...
        try {
            ...
            this.bind(window, 'resize', function () {
                ...
                this.resizeTimer = setTimeout(function () {
                    rs.template.tuwen_image_text_fulllunbo_6_5.page.containerResize();
                }, 100);
            });
        } catch (e) {
        }
    },
    bindChangePageHandler: function () {
        var nextPage = document.getElementById('arrowR');
        var prePage = document.getElementById('arrowL');
        this.bind(nextPage, 'click', this.onNextClick.proxy(this));
        this.bind(prePage, 'click', this.onPreClick.proxy(this));
    },
    // 绑定下载按钮事件
    bindCloseHandler: function () {
        var closeBtn = document.getElementById('closeBtn');
        this.bind(closeBtn, 'click', function () { rs.template.tuwen_image_text_fulllunbo_6_5.page.sdk.onAdPlayEnd(); lunbo.stop(); rs.template.tuwen_image_text_fulllunbo_6_5.page.stopEvent(this); });
    },
    ...
    adClick: function (containerId, adsExtention) {
        ...
        var liCon = container.getElementsByTagName('li');
    },
    // 广告被点击
    ...
    containerResize: function (event) {
        ...
        item.style.marginLeft = (conW - Math.floor(parseFloat(itemUl.clientWidth)) * 1.143) / 2 + 'px';
        item.style.marginTop = (conH - parseInt(itemUl.clientHeight, 10)) / 2 + 'px';
    },
