oojs.define({
    name: 'layout',
    namespace: 'rs.template.text_app_download_6_5',
    deps: {
    ...
    render: function (context) {
        ...
        var titWidth = itemWidth, decWidth = itemWidth, graWidth = itemWidth, dowWidth = itemWidth;
        ...
        // title字体尺寸颜色定义
        var titleFontSize = userConfig['titleFontSize'] && Number(userConfig['titleFontSize']) ? Math.round(Number(userConfig['titleFontSize']) * cpx) : Math.round(44 * px);
        ...
        userConfig['titleFontFamily'] && (style['.item .tit span.title']['font-family'] = userConfig['titleFontFamily']);
        // desc字体尺寸颜色定义
        var descFontSize = userConfig['descFontSize'] && Number(userConfig['descFontSize']) ? Math.round(Number(userConfig['descFontSize']) * cpx) : Math.round(30 * px);
        ...
        // 评级区文字
        var graFontSize = userConfig['appInfoFontSize'] && Number(userConfig['appInfoFontSize']) ? Math.round(Number(userConfig['appInfoFontSize']) * cpx) : Math.round(24 * px);
        ...
        // “免费下载”文字
        var dowFontSize = userConfig['appDownloadFontSize'] && Number(userConfig['appDownloadFontSize']) ? Math.round(Number(userConfig['appDownloadFontSize']) * cpx) : Math.round(28.93 * px);
        ...
        for (var i = 0; i < adCount; i++) {
            ...
            // 组装layoutObj
            item.childNodes.push(title, desc, grey);
            // 组装text
