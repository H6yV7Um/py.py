...
oojs.define({
    ...
    $page: function () {
        ...
        this.page2.classList.add('hack4uc');

        this.computeRootFontSize(750);
        // this.changeColorBottomHeight();
        this.initPage2();
        ...
        this.bind(downloadBtn, 'click', function () {
            ...
            self.page2.classList.remove('hack4uc');
            self.overlay.style.display = 'block';
            setTimeout(function () {
                self.overlay.style.display = 'none';
            }, 1000 * 15);
            // self.changeColorBottomHeight();
            self.computeRootFontSize(420);
            // document.querySelector('body').classList.add('special');
            // 跳转到第三页
            // self.page1.style.display = 'none';
            // self.computeRootFontSize(750);
            // self.page3.style.display = 'block';

            // 跳转到第四页
            // self.page1.style.display = 'none';
            // self.computeRootFontSize(750);
            // self.page4.style.display = 'block';
        });
    ...
    changeColorBottomHeight: function () {
        var viewportHeight = Math.max(document.documentElement.clientHeight, window.innerHeight || 0);
        var containerHeight = parseInt(window.getComputedStyle(document.querySelector('#container')).height, 10);
        var minColorHeight = 100;
        var colorHeight = viewportHeight - containerHeight + 10;
        document.querySelector('.color').style.height = (colorHeight <= 0 ? minColorHeight : colorHeight) + 'px';
    },
    ...
    adClick: function (containerId) {
        ...
        for (var i = 0; i < linkArray.length; i++) {
            ...
            if (tempClassName) {
                ...
                if (tempClassName === 'gylogo' || tempClassName === 'bdlogo' || tempClassName.substring(0, 7) === 'bd-logo' || linkArray[i].id.indexOf('downBtn') < 0) {
                    continue;
                }
            }
    ...
    bindPage2EventHandler: function () {
        ...
        //             var pageNum = self.getAttribute(element,'data-page');
    ...
    getTelNumber: function () {
        var phoneNumberInput = document.querySelector('.pass-text-input-mobilenum');
    ...
    checkAnswerHandler: function (result) {
        ...
        // body.style.backgroundSize = '10rem';
        this.computeRootFontSize(750);
    ...
    downloadBaidubaoHandler: function () {
        this.sendEclickRequest('http://qrcode-logs.baidu.com/ad.jpg', {
            url2: 'qrcode',
            pageNum: 4,
    ...
    passportOnloadCallback: function () {
        ...
        passport.use('smsLogin', {
            ...
            var loginInstance = new apiMagic.passport.smsLogin({
                ...
                // staticPage: 'http://test.baidu.com/v3Jump.html', // 跨域用静态页面
            ...
            function renderCallback() {
                ...
                phoneNumberInput.placeholder = '请输入手机号领取';
                ...
                tdArr[0].appendChild(phoneNumberInput);
                tdArr[1].appendChild(requirePasswordBtn);
                tdArr[2].appendChild(passwordInput);
                tdArr[3].appendChild(loginBtn);
